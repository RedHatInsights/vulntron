...
func main() {
	...
	syftOut, err := exec.Command("syft", "--scope", "all-layers", fmt.Sprintf(imageTag), "-o", "json=syftout.json").Output()
	if err != nil {
	...
	grypeOut, err := exec.Command("grype", "--scope", "all-layers", fmt.Sprintf(imageTag), "-o", "json", "--file", "grypeout.json").Output()
	if err != nil {
